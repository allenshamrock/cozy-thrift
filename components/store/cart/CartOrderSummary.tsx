...
export default function CartOrderSummary() {
  const variants = {
    ...
    aniamte: { opacity: 1, y: 0, transition: { duration: 0.5 } },
  };

  const selectOptions = ["Free delivery"];

  const cart = useCartStore((state) => state.cart);
  const cartTotal = cart.reduce((acc, currValue) => {
    ...
    >
      <div className="p-2 md:py-8 md:px-4 flex flex-col justify-center">
        ...
        <Button
          solid
          label="APPLY"
          className="my-6 w-fit bg-white text-primary"
        />
        <div className="flex items-center justify-between font-semibold md:text-lg uppercase mb-4 md:mb-8">
